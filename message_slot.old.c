...
static int device_release(struct inode * inode, struct file * file)
{
    ...
    if (file->private_data != NULL) {
        ...
        printk(KERN_DEBUG "kfree file->private_data, unfound node\n");
        printk(KERN_DEBUG "in %s, kfree address: %p\n", __func__, file->private_data);
...
static int alloc_node(node_t ** new_node) {
...
static node_t * query_list(device_node_t * device, uint32_t id) {
...
static device_node_t * query_list_list(uint minor) {
